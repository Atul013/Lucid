...
| Agent loop — safety hardening | 👀 In Review | Am4l-babu | feature/agent-safety-hardening | Phase 2 gap the generic API-key middleware doesn't cover: per-run cooldown to protect the LLM/Azure budget, persistent append-only audit trail of agent actions (drafts/proposals/todos/Telegram sends), input bounds on /twin/simulate (PR #47) |
